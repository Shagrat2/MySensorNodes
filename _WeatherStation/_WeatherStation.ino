...
//#define BMP_ON // uncomment out this line to enable BMP085 sensor
...
#define CALIBRATE_FACTOR 30 // amount of rain per rain bucket tip e.g. 5 is .05mm
#define DHT_LUX_DELAY 5000 //300000  //Delay in milliseconds that the DHT and LUX sensors will wait before sending data
...
//
  ...
  #define METRIC
  #define CHILD_ID_BMP_BARO 6
  #define CHILD_ID_BMP_TEMP 7

  Adafruit_BMP085 bmp = Adafruit_BMP085();      // Digital Pressure Sensor
  
  const float ALTITUDE = 688; // <-- adapt this value to your own location's altitude. 

  float lastBMPPressure = -1;
  float lastBMPTemp = -1;

  MyMessage tempMsg(CHILD_ID_BMP_TEMP, V_TEMP);
  MyMessage pressureMsg(CHILD_ID_BMP_BARO, V_PRESSURE);
#endif
//
#ifdef LUX_ON
...
#ifdef BMP_ON
  present(CHILD_ID_BMP_BARO, S_BARO);
  wait(DWELL_TIME);
  present(CHILD_ID_BMP_TEMP, S_TEMP);
  wait(DWELL_TIME);
#endif
...
void doBMP(void)
{
    ...
    if (temperature != lastBMPTemp) 
    {
        send(tempMsg.set(temperature, 1));
        lastBMPTemp = temperature;
    }

    if (pressure != lastBMPPressure) 
    {
        send(pressureMsg.set(pressure, 0));
        lastBMPPressure = pressure;
    }
}
#endif
//
